...
pub fn add_std_functions(variables: &mut VariableMap){
    add_function!("add", variables, params, only (var1: Float, var2: Float,) {
        Ok(Variable::Float(var1+var2))
    });
    add_function!("subtract", variables, params, only (var1: Float, var2: Float,){
        Ok(Variable::Float(var1-var2))
    });
    add_function!("multiply", variables, params, only (var1: Float, var2: Float,){
        Ok(Variable::Float(var1*var2))
    });
    add_function!("divide", variables, params, only (var1: Float, var2: Float,){
        Ok(Variable::Float(var1/var2))
    });
    add_function!("modulo", variables, params, only (var1: Float, var2: Float,){
        let divided = var1/var2;
        let result = var1 - var2*divided.floor();
        Ok(Variable::Float(result))
    });
    add_function!("or", variables, params, only (var1: Float, var2: Float,){
        Ok(Variable::Float(var1.abs()+var2.abs()))
    });
    add_function!("not", variables, params, only (var: Float,){
